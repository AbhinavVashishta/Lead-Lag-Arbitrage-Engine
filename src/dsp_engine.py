...
class WienerKhinchinDSP:
    ...
    def compute_cross_correlation(self, x: np.ndarray, y: np.ndarray) -> Tuple[float, float, np.ndarray]:
        ...
        #remove dc bias (Analog electronics truly coming in clutch)
        x_clean = x - np.mean(x)
        y_clean = y - np.mean(y)
        ...
        x_win = x_clean*hann_window
        y_win = y_clean*hann_window
        ...
        if normalization_denom == 0.0:
            ...
            rho = float(r_xy_shifted[max_idx] / normalization_denom)
